...
__version__ = "1.0.4"
__all__ = ['cross', 'dot','multiply','power',
			'concatenate','matrix', 
			'Matrix', 'MatrixError']
...
def multiply(a,b):
	""" Returns elementwise multiplication of matrices a,b """
	if a.shape != b.shape: 
		raise MatrixError('Invalid shapes for elementwise multiplication.')
...
def power(a,exp):
	""" Returns the result of applying exp to all elements in m """
	m = a.copy()
	for i in range(m.nrows):
		for j in range(m.ncols):
			m[i][j] = m[i,j] ** exp
...
def concatenate(mats):
	""" Returns the conactenation of all input Matrices"""
	if len(mats) < 2 or \
		not all([mats[i].shape[1]==mats[i+1].shape[1] for i in range(len(mats)-1)]): 
		raise MatrixError('Invalid shapes for concatenation.')
	
	m_grid = []
	for mat in mats:
		m_grid += mat.copy().grid
	return Matrix(m_grid)
...
class Matrix:
    ...
    def __init__(self, rows=1, cols=1, fill=0, shape=None):
		if shape:
			rows = shape[0]
			cols = shape[1]
		if type(rows) == int:
    ...
    def __repr__(self):
        return '%s(%s)' % (
            self.__class__.__name__, str(self.grid)
        )

    def __setitem__(self, key, item):
		if len(self.grid) == 1:
			self.grid[0][key] = item
		else:
			raise Exception('Assignment Not Supported for %s' % (str(self)))
	
    def __getitem__(self, slice_tuple):
		''' Support numpy style slicing '''	
		
		sliced = None
		if type(slice_tuple) in [int,float] or type(slice_tuple) == slice:
			sliced = self.grid[slice_tuple]
		elif len(slice_tuple) != 2:
			raise Exception('Syntax {} not supported.'.format(slice_tuple))
		elif all([type(e) in [int,float] for e in slice_tuple]):
			sliced = self.grid[slice_tuple[0]][slice_tuple[1]]
		elif type(slice_tuple[0]) in [int,float]:
			sliced = self.grid[slice_tuple[0]][slice_tuple[1]]
		elif all([type(e)==slice for e in slice_tuple]):
			sliced = []
			for row in self.grid[slice_tuple[0]]:
				sliced += [row[slice_tuple[1]]]
		elif type(slice_tuple[1]) in [int,float]: 
			sliced = []
			for i,row in enumerate(self.grid[slice_tuple[0]]):
				sliced += [[row[slice_tuple[1]]]]
		else:
			raise Exception('Syntax {} not supported.'.format(slice_tuple))

		# return a single elemtent from matrix as is
		if type(sliced) in [int,float]: return sliced
		
		# other wise convert it to a matrix 
		if type(sliced[0]) in [int,float]: sliced = [sliced]
		mat = Matrix(sliced)	
		return mat
    ...
    def __add__(self, other):
		if isinstance(other,Matrix):
			...
			for row, col, element in self.elements():
				m.grid[row][col] = element + other[row,col]
		elif type(other) in [int,float]:
			m = self.map(lambda x: x+other)
		else:
			raise MatrixError('cannot add %s from a matrix' % type(other))
		return m
    ...
    def __div__(self,other):
		if type(other) in [int,float]:
			return self.map(lambda x: x / float(other))
		else:
			raise MatrixError('incompatible division for other type %s)' % (type(other)))
    ...
    def ravel(self):
		""" Returns a flattened copy of matrix. """
		flat = [e for e in self]
		return Matrix.FromList([flat])
    ...
    def std(self):
		return power(self-self.mean(),2).mean() ** 0.5
