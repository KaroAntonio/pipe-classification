            ...
            if not self.attribute_type[attribute] == 'real':
                ...
                for attval in self.stat_attributes[attribute].keys():
                    for label in self.stat_attributes[attribute][attval]:
                        ...
                        self.model['real_stat'][attribute][label]['count'] += float(self.stat_attributes[attribute][attval][label])
            ...
            if not self.attribute_type[attribute] == 'real':
                ...
                for label in self.labels:
                    if attval in self.model['cprob'][attribute] and label in self.model['cprob'][attribute][attval] and self.model['cprob'][attribute][attval][label] > 0:
                        scores[label] *= float(self.model['cprob'][attribute][attval][label])
                    elif attribute in self.smoothing.keys():
                        scores[label] *= float(self.model['cprob'][attribute]['*'][label])
                    else:
                        scores[label] = 0
            else:
                for label in self.labels:
                    nscores[label] = float(self.gaussian / self.model['real_stat'][attribute][label]['sigma'] * exp(-0.5 * ((float(params['attributes'][attribute]) - self.model['real_stat'][attribute][label]['mean']) / self.model['real_stat'][attribute][label]['sigma']) ** 2))
